...
function! s:SetGrepPrg(sort)
  if ( &filetype == "awk" || &filetype == "c" || &filetype == "lisp" ||
		 \ &filetype == "php" || &filetype == "python" || &filetype == "ruby" ||
		 \ &filetype == "scheme" || &filetype == "sh" ||  &filetype == "slang" )
    return '/usr/local/bin/ctags -x --'.&filetype.'-types=f --sort='.a:sort
  elseif ( &filetype == "cpp"  )
    return '/usr/local/bin/ctags -x --c++-types=f --sort='.a:sort
  elseif ( &filetype == "expect" || &filetype == "tcl" )
    return '/usr/local/bin/ctags -x --language-force=tcl --sort='.a:sort
  elseif ( &filetype == "fortran"  )
    return '/usr/local/bin/ctags -x --'.&filetype.'-types=fs --sort='.a:sort
  elseif ( &filetype == "java" )
    return '/usr/local/bin/ctags -x --java-types=c --sort='.a:sort
	elseif ( &filetype == "make" )
    return '/usr/local/bin/ctags -x --make-types=m --sort='.a:sort
  elseif ( &filetype == "pascal"  )
    return '/usr/local/bin/ctags -x --'.&filetype.'-types=fp --sort='.a:sort
	elseif ( &filetype == "perl" || &filetype == "rexx" )
    return '/usr/local/bin/ctags -x --'.&filetype.'-types=s --sort='.a:sort
  elseif ( &filetype == "vim" )
    return '/usr/local/bin/ctags -x --language-force=vim --sort='.a:sort
  else
	  return "fail"
	endif
endfunction
