...
if !exists("s:loaded_drawit")
 ...
 map <silent> <unique> <script> <Plug>LoadDrawIt  :set lz<CR>:call <SID>LoadDrawIt()<CR>:set nolz<CR>

 " LoadDrawIt: this routine loads the rest of DrawIt
 fu! <SID>LoadDrawIt()
	runtime! plugin/DrawIt.vim
	let s:loaded_drawit= 2
   map <Leader>di <Plug>StartDrawIt
	set lz
...
function! <SID>StopDrawIt()
 ...
 if has("gui_running")
  ...
  exe 'menu   '.g:DrChipTopLvlMenu.'DrawIt.Start\ DrawIt		<Leader>di'
 endif
endfunction
